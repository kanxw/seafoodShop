<template>
  <div class="content-box">
    ...
    <rghxbatchCopy ref="batchCopy"></rghxbatchCopy>
  </div>
</template>
<script>
import {
  shopDiscountGetLayout,
  shopDiscountGetList,
} from "@/api/mange/shop/shopDiscount";
...
import rghxbatchCopy from "../components/batchCopy";
...
export default {
  ...
  methods: {
    ...
    getList() {
      return shopDiscountGetList(Object.assign(this.pageData, this.formData))
        .then((res) => {
      ...
      switch (btnName) {
        case "delete": {
          ...
          selectRow.forEach((item) => {
            tips +=
              `<br/>` + item["店铺名称"] + ": 指标类别为" + item["指标类别"];
            shops += item["Key"] + ",";
          });

          let tipscontent = `将删除已选中的${tips} ${selectRow.length}个指标，是否继续`;
