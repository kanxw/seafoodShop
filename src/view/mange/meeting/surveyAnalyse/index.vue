<template>
  <div class="content-box">
    ...
    >
      <template #tableTabs v-if="tableTabsData != ''">
        <rghxtabletabs
          :tabsData.sync="tableTabsData"
          @tabClick="tableTabsClick"
        ></rghxtabletabs>
      </template>
      <template #template v-if="!isShowTable">
        <rghxTemplate :tabData="tabData"></rghxTemplate>
      </template>
    </rghxtableList>
...
import {
  ...
  getListData,
  getUserID,
} from "@/api/mange/meeting/surveyAnalyse";
...
import rghxTemplate from "./components/template";
...
export default {
  ...
  methods: {
      ...
      return getListData(Object.assign(this.formData, this.pageData))
        .then((res) => {
          if (res.data.errormsg == "") {
            let tabindex = this.formData["other"]["tabindex"];
    ...
    tableTabsClick(val) {
      ...
      this.getList();
      this.$refs.search.formDataCreate();
    },
    componentshow() {},
  },
};
